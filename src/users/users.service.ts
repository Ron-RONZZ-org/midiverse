...
export class UsersService {
  ...
  async updateProfile(userId: string, updateUserDto: UpdateUserDto) {
    ...
    if (updateUserDto.email && user.lastEmailChange && user.lastEmailChange > fifteenDaysAgo) {
      const daysLeft = Math.ceil((user.lastEmailChange.getTime() - fifteenDaysAgo.getTime()) / (24 * 60 * 60 * 1000));
      throw new BadRequestException(`You can only change your email once every 15 days. Please wait ${daysLeft} more days.`);
    }

    // Check if username can be changed
    if (updateUserDto.username && user.lastUsernameChange && user.lastUsernameChange > fifteenDaysAgo) {
      const daysLeft = Math.ceil((user.lastUsernameChange.getTime() - fifteenDaysAgo.getTime()) / (24 * 60 * 60 * 1000));
      throw new BadRequestException(`You can only change your username once every 15 days. Please wait ${daysLeft} more days.`);
    }
